...
impl AppState {
    ...
    pub fn handle_app_event(&mut self, event: AppEvent) -> Vec<PaneStateUpdate> {
        match event {
            ...
            AppEvent::AgentPromptSubmissionFailed { .. } => Vec::new(),
    ...
    where
        F: FnOnce(&mut crate::terminal::TerminalState) -> Option<TerminalStateMutation>,
    {
        self.update_terminal_state_with_mutation_and_completion_policy(pane_id, false, update)
    }

    /// Keep the authority mutation even when its visible projection is unchanged.
    /// Transcript ownership and completion suppression share this acceptance boundary.
    fn update_terminal_state_with_mutation_and_completion_policy<F>(
        &mut self,
        pane_id: PaneId,
    ...
    pub(crate) fn publish_pane_process_exit_if_agent(
        ...
        let (update, mutation) = self.update_terminal_state_with_mutation_and_completion_policy(
            pane_id,
            suppress_completion,
            |terminal| {
