...
impl PaneTerminal {
    ...
    /// Search one bounded result around a validated opaque API cursor. Validation
    /// uses the same retained snapshot as navigation, never untrusted row ranges.
    pub(crate) fn search_text_window_for_match(
        &self,
        query: &str,
        case_sensitive: bool,
        direction: TerminalSearchDirection,
        ...
        let previous = previous.filter(|previous| {
            previous.scan_cols == buffer.cols
                && previous.scan_screen == active_screen
                && buffer.contains_match(*previous)
        });
        // Missing/stale anchors wrap to the first/last retained match, including
        // a match at (0, 0), unlike a TUI search starting at its visible cursor.
        let cursor = match direction {
            TerminalSearchDirection::Forward => TerminalTextPoint {
                row: u32::MAX,
                col: u16::MAX,
            },
            TerminalSearchDirection::Backward => TerminalTextPoint { row: 0, col: 0 },
        };
        buffer.search_window(
            query,
            case_sensitive,
            ...
            direction,
            cursor,
            previous.map(|previous| (previous.start, previous.end)),
            1,
        )
    }
