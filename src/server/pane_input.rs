...
fn apply_scroll(
    ...
    // Coalesced wheel counts must reach applications as one atomic input batch.
    // The u16 wire count bounds the repeated fixed-size event allocation.
...
mod tests {
    ...
    fn with_terminal_attach_page_key_runtime(
        ...
        test: impl FnOnce(&crate::terminal::TerminalRuntime, &mut tokio::sync::mpsc::Receiver<Bytes>),
    ) {
