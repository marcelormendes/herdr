...
`pane.search` は、`pane.read` の行数制限より古い履歴を含め、現在のターミナルに保持されたテキスト全体を検索し、一致箇所へネイティブの表示領域を移動します。Unicode を考慮したリテラル検索で、表示幅による折り返しをまたいで検索できます。明示的な改行は別の行として扱います。アプリケーションが代替画面を使用中なら、その画面を検索します。
...
必須の `terminal_id` はペインの再接続先を検証し、不一致なら `terminal_mismatch` を返します。クエリの上限は UTF-8 で 4096 バイトです。`case_sensitive` の既定値は `false` です。`first` は最も古い一致を選択します。返された不透明な `cursor` と `next` / `previous` を使うと、端で折り返しながら移動できます。

無効または古くなったカーソルでは移動をやり直します（`next` は最も古い一致、`previous` は最も新しい一致から開始）。16384 バイトを超えるカーソルは `invalid_params` になります。クエリ、大文字小文字の区別、ターミナルの変更はカーソルを無効にします。カーソルは現在のテキストと座標を参照し、履歴上の出現箇所を永続的に識別するものではありません。履歴の破棄後に同じテキストが同じ座標に現れると、区別できない場合があります。各リクエストで現在の保持バッファを再検索します。

レスポンスは `{"type":"pane_search","search":{...}}` で、`pane_id`、`terminal_id`、`query`、`case_sensitive`、`match_count`、0 始まりの `match_index`、`cursor`、`preview` を含みます。プレビューは選択された行の文脈で、UTF-8 で最大 16384 バイトです。一致がない場合、インデックス、カーソル、プレビューはすべて `null` で、表示領域は変わりません。空のクエリも同じ動作です。移動前に出力が一致箇所を書き換えた場合は `search_changed` を返すため、リクエストを再試行してください。検証と表示領域の移動はターミナルへの書き込みに対して不可分に行われます。
